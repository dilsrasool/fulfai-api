...
public class OrderService {
    ...
    public void updateOrderStatus(String companyId, String orderId, String newStatus) {
        ...
        try {
            ...
            // If order is delivered, update account balance
    ...
    /**
     * Update account balance when order is delivered.
     */
    private void updateAccountBalanceOnDelivery(String companyId, String orderId) {
        Order order = orderRepository.getById(companyId, orderId);
        if (order != null && order.getTotalAmount() != null) {
            accountService.addToBalance(companyId, null, order.getTotalAmount(), orderId);
            Log.debugf("Updated account balance for delivered order: %s, amount: %s",
    ...
    public void acceptOrderWithStockReduction(String companyId, String orderId) {
        ...
        if (order.getBranchId() == null) {
            throw new BadRequestException("Order missing branchId");
        }
        ...
        for (OrderItem item : order.getItems()) {
            ...
            Product product = productRepository.getById(companyId, branchId, item.getProductId());
            if (product == null) {
